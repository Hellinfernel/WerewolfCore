...
public class GamePlayerAsync implements PlayerAsync {
    private final GameRole _role;
    private final UserAsync _user;
    ...
    public GamePlayerAsync(final GameAsync game,final GameRole role, final UserAsync user) {
        _role = role;
        _user = user;
        _game = game;
    }
    public GamePlayerAsync(final GameAsync game,final GameRole role, final UserAsync user,EnumSet<SpecialRole> specialRoles) {
        _role = role;
        _user = user;
        _specialRoles.addAll(specialRoles);
        _game = game;
    }
    public GamePlayerAsync(final GameAsync game,final GameRole role, final UserAsync user,SpecialRole specialRoles) {
        _role = role;
        _user = user;
